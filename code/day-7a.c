...
void calculate_sum_of_sizes(struct dir_structure * current, int treshold, int * total) {
    for(int i = 0; i < current->num_children; i++) {
        calculate_sum_of_sizes(current->children[i], treshold, total);
    }
...
int main(int argc, char const *argv[]) {
    ...
    while ((chars_read = getline(&line, &len, fp)) != -1) {
        ...
        if(strncmp(line, "$ ", 2) == 0) {
            ...
            if(strncmp(line, "$ cd ", 5) == 0) {
                ...
                printf("changed current working dir to: %s\n", working_dir->name);
            }
    ...
    int sum;
    calculate_sum_of_sizes(root, 100000, &sum);
